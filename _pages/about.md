...
**1. Exploring Deep Learning for Long-sequence Time-Series Forecasting (LSTF)**: My focus is on developing sophisticated neural network architectures (such as DNN based on Linear, seq2seq and eco-dec structures) that can accurately predict energy demands and renewable energy supply over mid/long term. This involves tackling the inherent challenges of long-sequence forecasting. At the same time, adopting multi-task learning technique, these models can also be applied to many classical downstream tasks, such as economic dispatch, etc.
